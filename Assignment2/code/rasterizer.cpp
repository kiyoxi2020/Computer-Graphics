...
{   
    ...
    for(int i=0; i<=2; i++){
        ...
        a << x-_v[j].x(), y-_v[j].y();
        Eigen::Vector2f b;
        b << _v[i].x()-_v[j].x(), _v[i].y()-_v[j].y();
        bool t = (a.x()*b.y()-a.y()*b.x())>0;
        if(i==0) z=t;
        else{
            if(z!=t) return false;
        }
...
void rst::rasterizer::rasterize_triangle(const Triangle& t) {
    ...
    for(int x=x_min; x<x_max; x++){
        for(int y=y_min; y<y_max; y++){
            count = 0;
            for(float dx=-0.25; dx<=0.25; dx=dx+0.5){
                for(float dy=-0.25; dy<=0.25; dy=dy+0.5){
            ...
            if(count>0){
                ...
                if(z_interpolated > depth_buf[ind]){
                    ...
                    if(count<4) depth_buf[ind] = std::numeric_limits<float>::min();
                }
